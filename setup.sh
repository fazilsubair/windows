...
readonly VM_NAME="WinApps"  # Name of the Windows VM (FOR 'libvirt' ONLY).
readonly RDP_PORT=3389         # Port used for RDP on Windows.
readonly DOCKER_IP="172.17.0.1" # Localhost.
...
# WINAPPS CONFIGURATION FILE
RDP_USER="MyWindowsUser"        # Imported variable.
RDP_PASS="MyWindowsPassword"        # Imported variable.
RDP_DOMAIN=""      # Imported variable.
RDP_IP="172.17.0.1"          # Imported variable.
...
function waCheckPortOpen() {
    ...
    if [ -z "$RDP_IP" ] && [ "$WAFLAVOR" = "libvirt" ]; then
        ...
        RDP_IP="127.0.0.1"         # VM IP address.
